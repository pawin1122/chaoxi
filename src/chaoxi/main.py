...
from chaoxi.__version__ import __version__
...
def _version_callback(value: bool) -> None:
    if value:
        Console().print(f"chaoxi v{__version__}")
        raise typer.Exit()
...
def chaoxi(
    ctx: typer.Context,
    version: bool | None = Option(None, "--version", callback=_version_callback, is_eager=True, help="显示版本号"),
    ...
    if config.industry:
        Console().print(
            "[yellow]⚠ 行业筛选（--industry）功能尚未实现，本次查询将忽略此参数。[/yellow]"
        )
