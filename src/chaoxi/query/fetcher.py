...
class AnnouncementFetcher:
    ...
    async def fetch(self, org_ids: dict[str, dict]) -> list[RawAnnouncement]:
        ...
        shards = self._compute_date_shards(self._config.start_date, self._config.end_date)
        ...
        for shard_start, shard_end in shards:
            se_date = f"{shard_start}~{shard_end}"
            column_tasks = [
            ...
            if show_progress:
                count = len(results)
                print(f"\r查询中 {count} 条...", end="", flush=True)
    ...
    def _compute_date_shards(self, start: date, end: date) -> list[tuple[date, date]]:
        return [(start, end)]
